...
## 2026-11-08 - Missing Cross-Origin Isolation Headers
**Vulnerability:** The application was missing `Cross-Origin-Opener-Policy` (COOP) and `Cross-Origin-Resource-Policy` (CORP) security headers.
**Learning:** These headers are critical to establish "cross-origin isolation" and mitigate Cross-Site Leaks (XS-Leaks) and side-channel attacks like Spectre.
**Prevention:** Added `Cross-Origin-Opener-Policy: same-origin` and `Cross-Origin-Resource-Policy: same-origin` to `SecureHeadersMiddleware`.
## 2026-11-09 - DoS Risk via Unbounded Extra Parameters in API Requests
**Vulnerability:** The API request models (`TrimRequest`, `AnalysisRequest`, `AircraftParameters`) allowed arbitrary extra fields in the JSON payload because Pydantic V2 allows them by default (`extra='ignore'`). Attackers could submit massive amounts of unrecognized JSON keys to bypass the 1MB payload size limit indirectly, consuming CPU during JSON parsing and mass assignment, or exposing the application to mass-assignment vulnerabilities.
**Learning:** Strict input validation ("Defense in Depth") requires forbidding extra, unexpected data at the API boundary. This ensures attackers cannot inflate payload parsing times by inserting massive nested garbage objects inside allowed objects, mitigating DoS vectors that operate within the total `Content-Length` limit but stress the parser.
**Prevention:** Added `model_config = ConfigDict(extra="forbid")` to `AircraftParameters`, `TrimRequest`, and `AnalysisRequest` models. This immediately rejects payloads with unknown keys returning a `422 Unprocessable Entity` ("extra_forbidden"), preventing mass assignment and parser exhaustion.
## 2026-05-15 - [Enhance IP anonymization with salted hashes]
**Vulnerability:** The application was hashing raw IP addresses (e.g., from `X-Forwarded-For` or `request.client.host`) without a salt before using them as keys in the rate-limiting dictionary. Since the IPv4 address space is relatively small (~4.3 billion addresses), these unsalted hashes are highly susceptible to rainbow table attacks, allowing an attacker with memory access or crash dumps to reverse the hashes and expose user PII.
**Learning:** Hashing alone is insufficient for anonymizing predictably small datasets like IPv4 addresses. A salt is required to ensure cryptographic operations prevent precomputation attacks, particularly when state is stored in long-lived memory structures like a globally scoped `defaultdict`.
**Prevention:** Added a globally generated salt (`secrets.token_hex(16)`) at application startup and concatenated it with the client IP before hashing. This ensures hashes are unique per application run and effectively immune to rainbow table attacks without adding dependency overhead.
