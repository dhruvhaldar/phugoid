...
## 2026-05-21 - Plotly CSP Constraint
**Vulnerability:** Weak CSP `style-src` allowing `'unsafe-inline'`.
**Learning:** Plotly.js injects inline styles for graph positioning, requiring `'unsafe-inline'` in `style-src`. Removing it breaks the charts.
**Prevention:** Accept this risk for now or use a nonce-based approach (complex for this setup). Do not attempt to remove `'unsafe-inline'` from `style-src` without replacing Plotly or implementing nonces.
