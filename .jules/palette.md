...
## 2026-03-05 - [Form Input Race Conditions]
**Learning:** Not disabling form inputs during async operations (like API calls) creates a race condition where users can modify the input values before the response returns. In engineering applications, this can cause the rendered output to visually desync from the current input values, causing confusion.
**Action:** Always comprehensively disable all relevant inputs (`input`, `.preset-btn`) before initiating an async call, and re-enable them in the `finally` block to ensure consistent state and visual feedback.
