...
## 2026-05-10 - TrimSolver Jacobian State Reuse
**Learning:** When calculating the finite-difference Jacobian in `TrimSolver`, the `alpha` value remains constant when computing the partial derivatives with respect to `elevator` and `throttle`. The original code blindly recalculated the full `state_tup` (which involves expensive `math.cos(alpha)` and `math.sin(alpha)` operations) for all three forward difference steps.
**Action:** When calculating finite-difference Jacobians, explicitly reuse pre-calculated state vectors for partial derivatives where the underlying state variables remain constant. This eliminates redundant trigonometric calculations and object initialization overhead, measurably improving performance.
