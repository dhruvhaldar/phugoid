...
class RequestSizeLimitMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if "chunked" in request.headers.get("transfer-encoding", "").lower():
            ...
            logger.warning(f"Security Event: Rejected chunked transfer encoding from {request.client.host if request.client else 'unknown'} on path {repr(str(request.url.path))}")
        ...
        if content_length:
            try:
                ...
                if length > 1048576:
                    logger.warning(f"Security Event: Payload too large ({length} bytes) from {request.client.host if request.client else 'unknown'} on path {repr(str(request.url.path))}")
                    return JSONResponse(status_code=413, content={"detail": "Payload Too Large"})
            except ValueError:
                logger.warning(f"Security Event: Invalid Content-Length header ('{content_length}') from {request.client.host if request.client else 'unknown'} on path {repr(str(request.url.path))}")
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        if len(timestamps) >= 100:
            ...
            logger.warning(f"Security Event: Rate limit exceeded for IP {client_ip} on path {repr(str(request.url.path))}")
